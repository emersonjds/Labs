...
let StartCommand = class StartCommand extends cli_utils_1.Command {
    preRun(inputs, options) {
        return tslib_1.__awaiter(this, void 0, void 0, function* () {
            ...
            if (proAppId && config.backend !== cli_utils_1.BACKEND_PRO) {
                yield this.runcmd(['config', 'set', '-g', 'backend', 'pro'], { showExecution: false, showLogs: false });
                this.env.log.nl();
    ...
    run(inputs, options) {
        return tslib_1.__awaiter(this, void 0, void 0, function* () {
            ...
            if (!projectExists) {
                ...
                if (confirm) {
                    try {
                        ...
                        const rimrafp = cli_utils_1.promisify(rimraf);
                    ...
                    this.env.log.info(`Not erasing existing project in ${chalk.green(cli_utils_1.prettyPath(projectRoot))}.`);
                    return 0;
                }
            }
            ...
            yield start_1.tarXvfFromUrl(wrapperBranchPath, projectRoot, { progress: (loaded, total) => {
            ...
            yield start_1.tarXvfFromUrl(starterBranchPath, extractDir, { progress: (loaded, total) => {
            ...
            if (options['type'] === 'ionic1') {
                ...
                yield start_1.tarXvfFromUrl(resourcesPath, path.join(projectRoot, 'resources'), { progress: (loaded, total) => {
            ...
            yield start_1.patchPackageJsonForCli(this.env, safeProjectName, starterType, projectRoot);
            yield start_1.updatePackageJsonForCli(this.env, safeProjectName, starterType, projectRoot);
            this.env.tasks.next(`Creating configuration file ${chalk.bold('ionic.config.json')}`);
            yield start_1.createProjectConfig(appName, starterType, projectRoot);
            this.env.tasks.end();
            if (options['deps']) {
                ...
                const [, ...dedupeArgs] = yield cli_utils_1.pkgManagerArgs(this.env, { command: 'dedupe' });
            ...
            if (config.backend === cli_utils_1.BACKEND_PRO) {
                ...
                if (linkConfirmed) {
                    this.env.project = new cli_utils_1.Project(projectRoot, cli_utils_1.PROJECT_FILE);
                    const cmdArgs = ['link'];
                    if (proAppId) {
            ...
            this.env.log.msg(`Go to your newly created project: ${chalk.green(`cd ${cli_utils_1.prettyPath(projectRoot)}`)}`);
